...
func TestLib(t *testing.T) {
	...
	c.Run("TestReadZipReaderWithFileWithNoWorkbookRels", func(c *qt.C) {
	...
	c.Run("TestReadZipReaderWithFileWithNoWorksheets", func(c *qt.C) {
	...
	c.Run("TestReadFileWithHyperlinks", func(c *qt.C) {
		file, err := OpenFile("./testdocs/file_with_hyperlinks.xlsx")
		if err != nil {
			c.Failed()
		}
		c.Assert(file.Sheets[0].Row(0).GetCell(0).Hyperlink, qt.Equals, Hyperlink{Link: "https://www.google.com/"})
		c.Assert(file.Sheets[0].Row(1).GetCell(0).Hyperlink, qt.Equals, Hyperlink{Link: "https://docs.microsoft.com/en-us/previous-versions/office/developer/office-2010/cc802445(v%3Doffice.14)"})
	})

	// Attempt to read data from a file with inlined string sheet data.
	c.Run("TestReadWithInlineStrings", func(c *qt.C) {
		var xlsxFile *File
		var err error
	...
	c.Run("TestReadWorkbookRelationsFromZipFileWithFunnyNames", func(c *qt.C) {
		var xlsxFile *File
		var err error
	...
	c.Run("TestWorkBookRelsMarshal", func(c *qt.C) {
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Target="worksheets/sheet.xml" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet"></Relationship><Relationship Id="rId2" Target="sharedStrings.xml" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings"></Relationship><Relationship Id="rId3" Target="theme/theme1.xml" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme"></Relationship><Relationship Id="rId4" Target="styles.xml" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles"></Relationship></Relationships>`
	...
	c.Run("TestSmooshBase26Slice", func(c *qt.C) {
		input := []int{20, 0, 1}
	...
	c.Run("TestFormatColumnName", func(c *qt.C) {
	...
	c.Run("TestGetLargestDenominator", func(c *qt.C) {
	...
	c.Run("TestLettersToNumeric", func(c *qt.C) {
	...
	c.Run("TestNumericToLetters", func(c *qt.C) {
	...
	c.Run("TestLetterOnlyMapFunction", func(c *qt.C) {
	...
	c.Run("TestIntOnlyMapFunction", func(c *qt.C) {
		var input string = "ABC123"
		var output string = strings.Map(intOnlyMapF, input)
		c.Assert(output, qt.Equals, "123")
	})

	c.Run("TestGetCoordsFromCellIDString", func(c *qt.C) {
	...
	c.Run("TestCalculateMaxMinFromWorksheet", func(c *qt.C) {
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
  ...
  <drawing r:id="rId1"/>
	...
	c.Run("TestGetRangeFromString", func(c *qt.C) {
	...
	c.Run("TestMakeRowFromSpan", func(c *qt.C) {
		var rangeString string
		var row *Row
		var length int
		var sheet *Sheet
	...
	c.Run("TestReadRowsFromSheet", func(c *qt.C) {
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
  ...
  <pageMargins left="0.7" right="0.7"
	...
	c.Run("TestReadRowsFromSheetBadR", func(c *qt.C) {
		var sharedstringsXML = bytes.NewBufferString(`
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="4" uniqueCount="4">
  <si>
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
  ...
  <pageMargins left="0.7" right="0.7"
	...
	c.Run("TestReadRowsFromSheetWithLeadingEmptyRows", func(c *qt.C) {
...
`)
		...
		c.Assert(maxRows, qt.Equals, 5)
		c.Assert(maxCols, qt.Equals, 1)

		c.Assert(rows[0].cellCount, qt.Equals, 0)
		c.Assert(rows[1].cellCount, qt.Equals, 0)
		c.Assert(rows[2].cellCount, qt.Equals, 0)
		c.Assert(rows[3].cellCount, qt.Equals, 1)
		if val, err := rows[3].GetCell(0).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "ABC")
		}
		c.Assert(rows[4].cellCount, qt.Equals, 1)
		if val, err := rows[4].GetCell(0).FormattedValue(); err != nil {
			c.Error(err)
		} else {
	...
	c.Run("TestReadRowsFromSheetWithLeadingEmptyCols", func(c *qt.C) {
		var sharedstringsXML = bytes.NewBufferString(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="2" uniqueCount="2"><si><t>ABC</t></si><si><t>DEF</t></si></sst>`)
		var sheetxml = bytes.NewBufferString(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac">
...
`)
		...
		rows, cols, maxCols, maxRows := readRowsFromSheet(worksheet, file, sheet, NoRowLimit)
		c.Assert(maxRows, qt.Equals, 2)
		c.Assert(maxCols, qt.Equals, 4)

		c.Assert(rows[0].cellCount, qt.Equals, 4)
		if val, err := rows[0].GetCell(0).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "")
		}
		if val, err := rows[0].GetCell(1).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "")
		}
		if val, err := rows[0].GetCell(2).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "ABC")
		}
		if val, err := rows[0].GetCell(3).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "DEF")
		}
		c.Assert(rows[1].cellCount, qt.Equals, 4)
		if val, err := rows[1].GetCell(0).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "")
		}
		if val, err := rows[1].GetCell(1).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "")
		}
		if val, err := rows[1].GetCell(2).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "ABC")
		}
		if val, err := rows[1].GetCell(3).FormattedValue(); err != nil {
			c.Error(err)
		} else {
			c.Assert(val, qt.Equals, "DEF")
		}

		c.Assert(cols.Len, qt.Equals, 2)
		c.Assert(cols.FindColByIndex(1), qt.IsNil)
		c.Assert(cols.FindColByIndex(2), qt.IsNil)
		c.Assert(cols.FindColByIndex(3).Width, qt.Equals, 17.0)
		c.Assert(cols.FindColByIndex(4).Width, qt.Equals, 18.0)
	})

	c.Run("TestReadRowsFromSheetWithEmptyCells", func(c *qt.C) {
		var sharedstringsXML = bytes.NewBufferString(`
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
`)
		...
		c.Assert(err, qt.IsNil)
		rows, cols, maxCols, maxRows := readRowsFromSheet(worksheet, file, sheet, NoRowLimit)
		c.Assert(maxRows, qt.Equals, 3)
		c.Assert(maxCols, qt.Equals, 3)
	...
	c.Run("TestReadRowsFromSheetWithTrailingEmptyCells", func(c *qt.C) {
		var row *Row
...
`)
		...
		c.Assert(maxCol, qt.Equals, 4)
		c.Assert(maxRow, qt.Equals, 8)
	...
	c.Run("TestReadRowsFromSheetWithMultipleSpans", func(c *qt.C) {
		var sharedstringsXML = bytes.NewBufferString(`
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="4" uniqueCount="4">
  <si>
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
  ...
  <pageMargins left="0.7" right="0.7"
	...
	c.Run("TestReadRowsFromSheetWithMultipleTypes", func(c *qt.C) {
		var sharedstringsXML = bytes.NewBufferString(`
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="4" uniqueCount="4">
  <si>
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
  ...
  <pageMargins left="0.7" right="0.7"
	...
	c.Run("TestReadRowsFromSheetWithHiddenColumn", func(c *qt.C) {
		var sharedstringsXML = bytes.NewBufferString(`
		<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
	...
	c.Run("TestReadRowFromRaw", func(c *qt.C) {
		var rawRow xlsxRow
		var cell xlsxC
		var row *Row
	...
	c.Run("TestReadRowFromRawWithMissingCells", func(c *qt.C) {
		var rawRow xlsxRow
		var cell xlsxC
		var row *Row
	...
	c.Run("TestReadRowFromRawWithPartialCoordinates", func(c *qt.C) {
		var rawRow xlsxRow
		var cell xlsxC
		var row *Row
	...
	c.Run("TestSharedFormulas", func(c *qt.C) {
		var sheetxml = bytes.NewBufferString(`
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
           xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  ...
  <pageMargins left="0.7" right="0.7"
	...
	c.Run("TestSharedFormulasWithAbsoluteReferences", func(c *qt.C) {
		formulas := []string{
			"A1",
			"$A1",
			"A$1",
	...
	c.Run("TestFormulaForCellPanic", func(c *qt.C) {
		cell := xlsxC{R: "A1"}
	...
	c.Run("TestRowNotOverwrittenWhenFollowedByEmptyRow", func(c *qt.C) {
	...
	`)
		...
		c.Assert(rows[3].cellCount, qt.Equals, 1)
		c.Assert(rows[3].GetCell(0).Value, qt.Equals, "75")
	})

	// This was a specific issue raised by a user.
	c.Run("TestRoundTripFileWithNoSheetCols", func(c *qt.C) {
	...
	c.Run("TestReadRestEmptyRowsFromSheet", func(c *qt.C) {
