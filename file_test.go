...
func TestFile(t *testing.T) {
	...
	// struct.
	c.Run("TestOpenFile", func(c *qt.C) {
		var xlsxFile *File
		var err error
	...
	c.Run("TestPartialReadsWithFewSharedStringsOnlyPartiallyReads", func(c *qt.C) {
	...
	c.Run("TestPartialReadsWithLargeSharedStringsOnlyPartiallyReads", func(c *qt.C) {
		// This test verifies that a large file is only partially read when using a small row limit.
	...
	c.Run("TestPartialReadsWithFewerRowsThanRequested", func(c *qt.C) {
		rowLimit := 10
	...
	c.Run("TestOpenFileWithoutStyleAndSharedStrings", func(c *qt.C) {
		var xlsxFile *File
		var error error
	...
	c.Run("TestOpenFileWithChartsheet", func(c *qt.C) {
	...
	c.Run("TestReadSharedStringsFromZipFile", func(c *qt.C) {
		var xlsxFile *File
		var err error
		xlsxFile, err = OpenFile("./testdocs/testfile.xlsx")
		c.Assert(err, qt.IsNil)
	...
	c.Run("TestReadStylesFromZipFile", func(c *qt.C) {
		var xlsxFile *File
		var err error
	...
	c.Run("TestReadWorkbookRelationsFromZipFile", func(c *qt.C) {
		var xlsxFile *File
		var err error
	...
	c.Run("TestCreateFile", func(c *qt.C) {
		var xlsxFile *File
	...
	// themselves correct.
	c.Run("TestCreateSheet", func(c *qt.C) {
		var xlsxFile *File
		var err error
		var sheet *Sheet
		var row *Row
	...
	c.Run("TestAddSheet", func(c *qt.C) {
		var f *File
	...
	c.Run("TestAddSheetWithDuplicateName", func(c *qt.C) {
		f := NewFile()
	...
	c.Run("TestAddSheetWithEmptyName", func(c *qt.C) {
		f := NewFile()
		_, err := f.AddSheet("")
	...
	c.Run("TestAppendSheet", func(c *qt.C) {
		var f *File
	...
	c.Run("TestAppendSheetWithDuplicateName", func(c *qt.C) {
		f := NewFile()
		s := Sheet{}
	...
	c.Run("TestMaxSheetNameLength", func(c *qt.C) {
	...
	c.Run("TestNthSheet", func(c *qt.C) {
		var f *File
	...
	c.Run("TestInvalidSheetNameCharacters", func(c *qt.C) {
		f := NewFile()
	...
	c.Run("TestMarshalWorkbook", func(c *qt.C) {
		var f *File
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><fileVersion appName="Go XLSX"></fileVersion><workbookPr showObjects="all" date1904="false"></workbookPr><workbookProtection></workbookProtection><bookViews><workbookView showHorizontalScroll="true" showVerticalScroll="true" showSheetTabs="true" tabRatio="204" windowHeight="8192" windowWidth="16384" xWindow="0" yWindow="0"></workbookView></bookViews><sheets><sheet name="MyFirstSheet" sheetId="1" r:id="rId1" state="visible"></sheet><sheet name="MySecondSheet" sheetId="2" r:id="rId2" state="visible"></sheet></sheets><definedNames></definedNames><calcPr iterateCount="100" refMode="A1" iterateDelta="0.001"></calcPr></workbook>`
	...
	c.Run("TestMarshalFile", func(c *qt.C) {
		var f *File
		f = NewFile()
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="1"><font><sz val="11"/><name val="Arial"/><family val="2"/><color theme="1" /><scheme val="minor"/></font></fonts><fills count="2"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill></fills><borders count="1"><border><left/><right/><top/><bottom/></border></borders><cellStyleXfs count="1"><xf applyAlignment="0" applyBorder="0" applyFont="0" applyFill="0" applyNumberFormat="0" applyProtection="0" borderId="0" fillId="0" fontId="0" numFmtId="0"><alignment horizontal="general" indent="0" shrinkToFit="0" textRotation="0" vertical="bottom" wrapText="0"/></xf></cellStyleXfs><cellXfs count="1"><xf applyAlignment="0" applyBorder="0" applyFont="0" applyFill="0" applyNumberFormat="0" applyProtection="0" borderId="0" fillId="0" fontId="0" numFmtId="0"><alignment horizontal="general" indent="0" shrinkToFit="0" textRotation="0" vertical="bottom" wrapText="0"/></xf></cellXfs></styleSheet>`
	...
	c.Run("TestSaveFile", func(c *qt.C) {
		...
		c.Assert(ok, qt.Equals, true)
		c.Assert(len(sheet1.Rows), qt.Equals, 1)
		row1 = sheet1.Rows[0]
	...
	c.Run("TestMarshalFileWithHyperlinks", func(c *qt.C) {
		var f *File
		f = NewFile()
		sheet1, _ := f.AddSheet("MySheet")
		row1 := sheet1.AddRow()
	...
	// We can save a File as a valid XLSX file at a given path.
	c.Run("TestSaveFileWithHyperlinks", func(c *qt.C) {
	...
	c.Run("TestReadWorkbookWithTypes", func(c *qt.C) {
		var xlsxFile *File
		var err error
		...
		c.Assert(len(sheet.Rows), qt.Equals, 8)
		c.Assert(sheet.Rows[0].cellCount, qt.Equals, 2)

		// string 1
		c.Assert(sheet.Rows[0].GetCell(0).Type(), qt.Equals, CellTypeString)
		if val, err := sheet.Rows[0].GetCell(0).FormattedValue(); err != nil {
			c.Error(err)
		} else {
		...
		// string 2
		c.Assert(sheet.Rows[1].GetCell(0).Type(), qt.Equals, CellTypeString)
		if val, err := sheet.Rows[1].GetCell(0).FormattedValue(); err != nil {
			c.Error(err)
		} else {
		...
		// integer
		c.Assert(sheet.Rows[2].GetCell(0).Type(), qt.Equals, CellTypeNumeric)
		intValue, _ := sheet.Rows[2].GetCell(0).Int()
		c.Assert(intValue, qt.Equals, 12345)

		// float
		c.Assert(sheet.Rows[3].GetCell(0).Type(), qt.Equals, CellTypeNumeric)
		floatValue, _ := sheet.Rows[3].GetCell(0).Float()
		c.Assert(floatValue, qt.Equals, 1.024)

		// Now it can't detect date
		c.Assert(sheet.Rows[4].GetCell(0).Type(), qt.Equals, CellTypeNumeric)
		intValue, _ = sheet.Rows[4].GetCell(0).Int()
		c.Assert(intValue, qt.Equals, 40543)

		// bool
		c.Assert(sheet.Rows[5].GetCell(0).Type(), qt.Equals, CellTypeBool)
		c.Assert(sheet.Rows[5].GetCell(0).Bool(), qt.Equals, true)

		// formula
		c.Assert(sheet.Rows[6].GetCell(0).Type(), qt.Equals, CellTypeNumeric)
		c.Assert(sheet.Rows[6].GetCell(0).Formula(), qt.Equals, "10+20")
		c.Assert(sheet.Rows[6].GetCell(0).Value, qt.Equals, "30")

		// error
		c.Assert(sheet.Rows[7].GetCell(0).Type(), qt.Equals, CellTypeError)
		c.Assert(sheet.Rows[7].GetCell(0).Formula(), qt.Equals, "10/0")
		c.Assert(sheet.Rows[7].GetCell(0).Value, qt.Equals, "#DIV/0!")
	})
...
func TestSliceReader(t *testing.T) {
	...
	c.Run("TestFileToSlice", func(c *qt.C) {
		output, err := FileToSlice("./testdocs/testfile.xlsx")
		c.Assert(err, qt.IsNil)
		fileToSliceCheckOutput(c, output)
	})

	c.Run("TestFileToSliceMissingCol", func(c *qt.C) {
	...
	c.Run("TestFileObjToSlice", func(c *qt.C) {
	...
	c.Run("TestFileToSliceUnmerged", func(c *qt.C) {
	...
	c.Run("TestFileWithEmptyRows", func(c *qt.C) {
	...
	c.Run("TestFileWithEmptyCols", func(c *qt.C) {
		f, err := OpenFile("./testdocs/empty_rows.xlsx")
		c.Assert(err, qt.IsNil)
